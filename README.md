...
function initLevel(level){
  ...
  // Första stenen = spawnstenen = grön
...
function draw(){
  ...
  for(let i=0;i<stones.length;i++){
    ...
    else if(i===0) ctx.fillStyle = 'green';                 // första stenen grön
    else ctx.fillStyle = stone.visited ? 'gray':'white';    // övriga stenar
  ...
  ctx.fillText(`Poäng: ${score}`,20,60);

  if(rainbowMessage){
    const text="🎉 Grattis! Du har klarat spelet! 🌈";
